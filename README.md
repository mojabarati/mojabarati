```diff
+ ┌──────────────────────────────────────────────────────┐
+ │                                                      │
+ │   Mohammad Javad Barati                              │
+ │   Product Manager who builds                         │
+ │                                                      │
+ │   product strategy + business logic                  │
+ │   + technical execution                              │
+ │                                                      │
+ └──────────────────────────────────────────────────────┘
```

### `guest@github:~$ profile.mission`

```diff
+ Turn ambiguity into product decisions.
+ Build enough to reduce uncertainty.
+ Ship the smallest useful thing.
+ Measure what actually changed.
```

[LinkedIn](https://www.linkedin.com/in/mohammadjavadbarati/) · [Email](mailto:mojabarati@gmail.com) · [GitHub](https://github.com/mojabarati)

---

### `guest@github:~$ impact.log`

```diff
+ [3 months]   POS MVP shipped
+ [300+]       vendor interviews
+ [30+]        B2B APIs shipped
+ [120B IRR]   loan issuance in 6 months
+ [-30%]       CRM support tickets
+ [+26%]       team velocity in 3 months
```

### `guest@github:~$ domains`

`marketplaces/` · `ai-products/` · `b2b/` · `payments/` · `apis/` · `pos-rms/` · `back-office-systems/`

```diff
+ I do not build to replace engineers.
+ I build to reduce product uncertainty, understand constraints,
+ prototype faster, and make better decisions with engineering teams.
```

---

### `guest@github:~$ toolbox`

```diff
+ SQL            + Postman          + TypeScript
+ JavaScript     + Python           + Go
+ Node.js        + Express          + PostgreSQL
+ Prisma         + Git              + GitHub
+ Figma          + Jira             + Azure DevOps
+ BPMN           + Swagger/OpenAPI  + AI-assisted prototyping
```

---

<details>
<summary><code>guest@github:~$ career.evidence</code></summary>

<br/>

#### `TapsiFood / Product Manager`

```diff
+ Built the company's first restaurant POS product from zero.
+ Shipped the MVP in 3 months and the first market-fit version in 6 months.
+ Used insights from 300+ vendor interviews and competitor analysis
+ to define the validated feature set.
```

#### `Tourism Bank / Product Manager`

```diff
+ Shipped 30+ B2B APIs from concept to market.
+ Helped establish a recurring Open Banking revenue stream.
+ Supported 120B IRR in loan issuance within the first 6 months
+ of integrated lending capabilities.
```

#### `Chargoon / Product Manager`

```diff
+ Reduced CRM support tickets by 30%.
+ Increased team velocity by 26% in 3 months.
+ Delivered a prioritized feature set 25% ahead of schedule.
```

</details>

<details>
<summary><code>guest@github:~$ pm-who-builds</code></summary>

<br/>

```text
PM-WHO-BUILDS

DESCRIPTION
    Product Manager with hands-on technical depth.
    I go one technical layer deeper when it improves the product decision.

CAPABILITIES
    inspect APIs instead of guessing
    query data to validate assumptions
    understand authentication, states and permissions
    prototype when discussions are too abstract
    separate product problems from implementation issues
    discuss technical trade-offs with engineering teams

OUTPUT
    less ambiguity
    better product decisions
```

</details>

---

### `guest@github:~$ principle`

```diff
+ Build to learn. Ship to measure.
```

`Product Manager` · `Technical Product Thinking` · `Hands-on Prototyping`

[LinkedIn](https://www.linkedin.com/in/mohammadjavadbarati/) · [Email](mailto:mojabarati@gmail.com) · [GitHub](https://github.com/mojabarati)
